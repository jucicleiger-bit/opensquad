...
// above. instanceUrl is NOT part of this credential: it's the one shared
// Evolution server, configured once via OPENSQUAD_EVOLUTION_ADMIN_URL in
// content-central-server.js, not stored per project. Only instanceName
// (always `opensquad-${projectId}`, never user-typed) and this project's
// own per-instance token (generated by Evolution itself on creation) live
// here.
export async function saveProjectWhatsAppInstance(projectId, input, targetDir = process.cwd(), now = new Date()) {
  if (!input?.instanceName || !input?.apiKey) {
    throw new Error('instanceName and apiKey are required');
  }
  const paths = getCentralPaths(targetDir, projectId);
  return withProjectLock(targetDir, projectId, async () => {
    const project = await loadProject(paths);
