...
- **Server crashes/restarts mid-generation**: `reconcileInterruptedGenerations` (`content-central.js:4504`) already sweeps every project on startup and clears any single-image item stranded `generating: true`; it was extended on 2026-08-27 to also sweep carousels (per-slide `generating` and the whole carousel's `status`) and to isolate one broken project directory so it can't abort reconciliation for every other project. This matters more here than for the standalone tab: an automatic carousel has no operator watching it generate, so a crash mid-slide with no recovery path would leave a calendar day permanently stuck with no way to notice or fix it short of a database dig.
